  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <Card className="border-border/60 bg-card/90 shadow-sm">
        ...
        <CardContent className="grid gap-4">
          ...
          <div className="flex gap-2">
            <Button type="button" variant="outline" className="rounded-full" onClick={resetForm}><Plus className="size-4" />Nueva</Button>
          </div>

          <div className="grid gap-2">
            {loading ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="size-4 animate-spin" />Cargando memorias...</div>
            ) : memorias.length > 0 ? (
              memorias.map((memoria) => (
                <button key={memoria.id} type="button" onClick={() => selectMemoria(memoria)} className={`rounded-2xl border px-4 py-3 text-left transition ${selectedMemoriaId === memoria.id ? 'border-primary/40 bg-primary/5' : 'border-border/50 bg-background hover:bg-muted/20'}`}>
                  <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-muted-foreground"><FileText className="size-3.5" />{memoria.tipo}</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">{memoria.titulo}</div>
                </button>
              ))
            ) : (
      ...
      <Card className="border-border/60 bg-card/90 shadow-sm">
        ...
        <CardContent className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[200px_240px_minmax(0,1fr)]">
            <div className="grid gap-1.5">
              <Label className="text-xs">Tipo</Label>
              <select value={tipo} onChange={(event) => setTipo(event.target.value as Memoria['tipo'])} className="h-10 rounded-xl border border-border/70 bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40">
            ...
            <div className="grid gap-1.5">
              ...
              <select value={partidaId} onChange={(event) => setPartidaId(event.target.value)} disabled={tipo !== 'partida'} className="h-10 rounded-xl border border-border/70 bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:opacity-60">
            ...
            <div className="grid gap-1.5">
              ...
              <Input value={titulo} onChange={(event) => setTitulo(event.target.value)} placeholder="Memoria descriptiva del proyecto" />
            </div>
          </div>

          <div className="grid gap-1.5">
            <Label className="text-xs">Contenido</Label>
            <Textarea value={contenido} onChange={(event) => setContenido(event.target.value)} placeholder="Describe alcance, criterios técnicos, proceso constructivo, condiciones especiales y soporte documental." className="min-h-[420px]" />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-full border border-border/60 px-3 py-2 text-xs uppercase tracking-wide text-muted-foreground">
              {(memorias.find((item) => item.id === selectedMemoriaId)?.status ?? 'borrador')}
            </div>
            <Button className="rounded-full" onClick={handleSave} disabled={saving}>{saving ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}Guardar memoria</Button>
            {selectedMemoriaId ? <Button type="button" variant="outline" className="rounded-full" onClick={() => void handleStatusChange('revisado')} disabled={(memorias.find((item) => item.id === selectedMemoriaId)?.status ?? 'borrador') !== 'borrador'}>Enviar a revisión</Button> : null}
            {selectedMemoriaId ? <Button type="button" variant="outline" className="rounded-full" onClick={() => void handleStatusChange('aprobado')} disabled={(memorias.find((item) => item.id === selectedMemoriaId)?.status ?? 'borrador') !== 'revisado'}>Aprobar</Button> : null}
            ...
            {selectedMemoriaId ? <Button type="button" variant="outline" className="rounded-full" onClick={() => setSelectedMemoriaId('')}><Pencil className="size-4" />Seguir editando como nueva</Button> : null}
          </div>
        </CardContent>
      </Card>
    </div>
